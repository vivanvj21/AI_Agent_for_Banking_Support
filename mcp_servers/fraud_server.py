...
def report_card_lost(user_id: str, card_id: str) -> dict:
    ...
    return _report_card_lost(user_id=user_id, card_id=card_id)
...
def report_fraud_transaction(user_id: str, transaction_id: str, reason: str = "") -> dict:
    ...
    return _report_fraud_transaction(user_id=user_id, transaction_id=transaction_id, reason=reason)


@mcp.tool()
def get_flagged_transactions(user_id: str) -> dict:
    """List all transactions currently flagged as fraud for the given user."""
    return _get_flagged_transactions(user_id=user_id)
