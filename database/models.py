...
class AccountModel(Base):
    ...
    cards: Mapped[list[CardModel]] = relationship(
        "CardModel", back_populates="account", cascade="all, delete-orphan"
    )
...
class TransactionModel(Base):
    ...
    merchant: Mapped[str | None] = mapped_column(VARCHAR(255), nullable=True)
...
class MemoryEntryModel(Base):
    ...
    user_id: Mapped[str | None] = mapped_column(VARCHAR(50), nullable=True, index=True)
    session_id: Mapped[str | None] = mapped_column(
        VARCHAR(64), nullable=True, index=True
    )
    memory_type: Mapped[str] = mapped_column(VARCHAR(50), nullable=False, index=True)
    content: Mapped[str] = mapped_column(TEXT, nullable=False)
    role: Mapped[str | None] = mapped_column(VARCHAR(20), nullable=True)
