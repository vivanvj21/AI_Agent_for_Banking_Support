...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('currency', sa.VARCHAR(length=10), nullable=False, server_default='INR'),
        sa.Column('is_active', sa.INTEGER(), nullable=False, server_default='1'),
        sa.CheckConstraint("account_type IN ('checking', 'savings', 'credit')", name='ck_accounts_account_type'),
    ...
    op.create_table(
        ...
        sa.CheckConstraint("status IN ('active', 'locked', 'reported_lost')", name='ck_cards_status'),
    ...
    op.create_table(
        ...
        sa.CheckConstraint("txn_type IN ('deposit', 'withdrawal', 'purchase', 'transfer', 'fee', 'interest')", name='ck_transactions_txn_type'),
        sa.ForeignKeyConstraint(['account_id'], ['accounts.account_id'], ondelete='CASCADE'),
    ...
    op.create_table(
        ...
        sa.Column('channel', sa.VARCHAR(length=20), nullable=False, server_default='cli'),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
    ...
    op.create_index('idx_vector_entity', 'vector_embeddings', ['entity_type', 'entity_id'])
