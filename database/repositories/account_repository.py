...
class AccountRepository(BaseRepository[AccountModel]):
    ...
    async def get_by_user_id(self, user_id: str) -> List[AccountModel]:
        stmt = select(AccountModel).where(AccountModel.user_id == user_id, AccountModel.is_active == 1)
        res = await self.session.execute(stmt)
        return list(res.scalars().all())

    async def get_user_account(self, user_id: str, account_id: str) -> Optional[AccountModel]:
